...
#6.Calculate the lowest paid CEO and the lowest paid worker
SELECT company_name, ceo_name, MIN(salary) , MIN(median_worker_pay) FROM `s&p_500_data`.ceo_data_pay_merged_sp500;

#7.Calculate the vairiation/PERCENTAGE difference IN CEO salary vs the median worker salary 
...
#8.Calculating ceo salary that is above the industry average
...
#9.Calculating the ceo salary that is below the industry average
SELECT company_name, ceo_name,industry,salary FROM `s&p_500_data`.ceo_data_pay_merged_sp500
WHERE salary < (SELECT AVG(salary) FROM `s&p_500_data`.ceo_data_pay_merged_sp500);

#10.An sql statement that calculates the values above the average in the column and calculates the respective percentage that is below
WITH t AS (
...
#11.Calculating the ceo salary in each industry
...
#12.calculating the industry with the hightest combined salaries
